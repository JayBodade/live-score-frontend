...
const AdminControl = ({ setCurrentBall, lastThreeOvers, setLastThreeOvers, setWickets, runs, setRuns, overNo, setOverNo, overValue, moveValue, setMoveValue, setOverValue, wickets, currentBall, overArr, setOverArr }) => {
    ...
    return (
        <div ref={myElementRef} className="current-over relative mb-10 w-full h-fit
    mt-2  ">
  ...
  <img src="./grayarrow.jpb-removebg-preview.png" className='w-8 z-1 h-8 lg:ml-2 xl:ml-3 m:w-6 sm:h-6 sm:-ml-2 sm:mt-[29px] mt-[26px] ml-2 text-gray' alt="" />
            ...
            <div className='border-2 sm:w-[85%] w-[95%]
      border-black -mt-12 z-50 mx-auto sm:p-4 p-2 h-fit bg-white'>
                <div className="flex justify-between">
                    {Object.keys(overValue).map((key, index) => {
                        let color = overValue[key] == '' ? 'white' : colors[overValue[key]].color;
                        let textColor = overValue[key] == '' ? 'black' : colors[overValue[key]].text;
                      
                        return <div key={key}> 
                        
                        <div className={`flex items-center  justify-center sm:w-16 w-9   text-${textColor} sm:h-16 h-9 border ${currentBall == key ? 'border-yellow-300 border-[8px] border-solid:' : 'border-black'}  rounded-full`} style={{
                            backgroundColor:color,transition: 'background-color 0.3s'
                        }}>
                            <span className='text-xl'>{overValue[key]}</span>
                        
                        
                </div>
                            <span className='text-red-500 300:text-xs  375:text-sm 425:text-lg'>{`${parseInt(key) + 1} ball`}</span>
                            {currentBall === index ? <img src="./downward-removebg-preview.png" width={40} height={30} className='-mt-6 300:-ml-[0.01px] sm:ml-3 sm:w-[40px] 300:w-[30px]  absolute mx-auto' alt="" /> : ''}
                        </div>
